// Skanly.Application/Features/Owners/Services/OwnerService.cs
using FluentValidation;
using Microsoft.AspNetCore.Http;
using Microsoft.Extensions.Logging;
using Skanly.Application.Common.Interfaces;          // ✅ فقط
...
public class OwnerService : IOwnerService
{
        ...
        CancellationToken ct = default)
    {
        ...
        // ✅ IIdentityService بدل UserManager
        ...
        var verificationStatus = verification?.Status switch
        {
            ...
            _ => owner.IsIdentityVerified ? "Verified" : "Not Submitted"
        };
        ...
        var totalEarnings = await _uow.Owners
            .GetTotalEarningsAsync(userId, ct);
        ...
        CancellationToken ct = default)
    {
        ...
        // ✅ IIdentityService بدل userManager.UpdateAsync
    ...
    public async Task<ServiceResult> SubmitIdentityVerificationAsync(
        ...
        CancellationToken ct = default)
    {
        // ✅ لا يحتاج UserManager هنا أصلاً
        ...
        CancellationToken ct = default)
    {
        ...
        var monthBookings = await _uow.Bookings
            .GetConfirmedAsync(monthStart, now, ct);

        var monthlyEarnings = monthBookings
    ...
    public async Task<ServiceResult<PagedResult<OwnerPropertySummaryDto>>>
        GetPropertiesAsync(
            string ownerId,
            int pageNumber = 1,
            int pageSize = 10,
            ...
            CancellationToken ct = default)
    {
        ...
        {
            "Approved" => allProperties.Where(
                                p => p.IsApproved && !p.IsDeleted),
            "Pending" => allProperties.Where(
                                p => !p.IsApproved && !p.IsDeleted),
            "Unavailable" => allProperties.Where(
                                p => !p.IsAvailable && !p.IsDeleted),
        ...
        return ServiceResult<PagedResult<OwnerPropertySummaryDto>>.Success(
            PagedResult<OwnerPropertySummaryDto>.Create(
                dtos, total, pageNumber, pageSize));
    }

    // ── GetBookingRequestsAsync ───────────────────────────────────────────────

    public async Task<ServiceResult<PagedResult<BookingRequestDto>>>
        GetBookingRequestsAsync(
            string ownerId,
            int pageNumber = 1,
            int pageSize = 10,
            ...
            CancellationToken ct = default)
    {
        ...
        return ServiceResult<PagedResult<BookingRequestDto>>.Success(
            PagedResult<BookingRequestDto>.Create(
                dtos, total, pageNumber, pageSize));
    }

    // ── GetBookingRequestDetailAsync ──────────────────────────────────────────

    public async Task<ServiceResult<BookingRequestDto>>
        GetBookingRequestDetailAsync(
            string ownerId,
            int bookingId,
            CancellationToken ct = default)
        ...
        CancellationToken ct = default)
    {
        ...
        var totalEarnings = await _uow.Owners
            .GetTotalEarningsAsync(ownerId, ct);

        var now = DateTime.UtcNow;
        var monthStart = new DateTime(now.Year, now.Month, 1);
        var monthlyEarnings = confirmedBookings
    ...
    public async Task<ServiceResult<PagedResult<NotificationDto>>>
    ...
    private static BookingRequestDto MapToBookingRequestDto(Booking b) => new()
    {
        BookingId = b.Id,
    ...
    private async Task<IReadOnlyList<MonthlyEarningsPoint>>
        BuildEarningsChartAsync(
            string ownerId,
            int months,
            CancellationToken ct)
    {
        ...
        for (int i = months - 1; i >= 0; i--)
        {
            ...
            var bookings = await _uow.Bookings
                .GetConfirmedAsync(mStart, mEnd, ct);
